...
// ============================================================================
// Name generation (expanded from packages/lyntty-app/sources/utils/generateWorktreeName.ts)
// ============================================================================
...
export async function createEnvironment(opts?: { noSwitch?: boolean }): Promise<string> {
    ...
    const standaloneTs = path.join(REPO_ROOT, "packages", "lyntty-relay", "sources", "standalone.ts");
    const result = spawnSync(
        "tsx",
        [standaloneTs, "migrate"],
        {
            cwd: path.join(REPO_ROOT, "packages", "lyntty-relay"),
...
export async function startEnvironmentServices(name: string): Promise<void> {
    ...
    const webPid = spawnService("pnpm", ["web", "--port", String(config.expoPort)], {
        cwd: path.join(REPO_ROOT, "packages", "lyntty-app"),
...
export async function seedEnvironment(name: string): Promise<void> {
    ...
    const lynttyBin = path.join(REPO_ROOT, "packages", "lyntty-cli", "bin", "lyntty.mjs");
    const daemon = spawn("node", [lynttyBin, "daemon", "start"], {
        env: daemonEnv,
        stdio: "ignore",
        detached: true,
    });
...
function commandRun(service: string, serviceArgs: string[] = []) {
    ...
    switch (service) {
        ...
        case "cli": {
            ...
            const cliBin = path.join(REPO_ROOT, "packages", "lyntty-cli", "bin", "lyntty.mjs");
            const result = spawnSync(
                "node",
                [cliBin, ...serviceArgs],
                {
...
function buildEnvVars(envDir: string, serverPort: number, expoPort: number): Record<string, string> {
    ...
    return {
        // Server
        HANDY_MASTER_SECRET: "lyntty-dev-secret",
        ...
        EXPO_PUBLIC_LYNTTY_SERVER_URL: `http://localhost:${serverPort}`,
        EXPO_PUBLIC_LOG_SERVER_URL: "http://localhost:8787",
        EXPO_PORT: String(expoPort),

        // CLI
        LYNTTY_SERVER_URL: `http://localhost:${serverPort}`,
        LYNTTY_WEBAPP_URL: `http://localhost:${expoPort}`,
        LYNTTY_HOME_DIR: path.join(envDir, "cli", "home"),
        LYNTTY_PROJECT_DIR: projectDir,
        LYNTTY_VARIANT: "dev",
        DEBUG: "1",
        ...(devAuth ? {
...
function buildEnvSh(name: string, envDir: string, serverPort: number, expoPort: number): string {
    ...
    lines.push(`export EXPO_PUBLIC_LYNTTY_SERVER_URL="${vars.EXPO_PUBLIC_LYNTTY_SERVER_URL}"`);
    ...
    lines.push("# CLI");
    lines.push(`export LYNTTY_SERVER_URL="${vars.LYNTTY_SERVER_URL}"`);
    lines.push(`export LYNTTY_WEBAPP_URL="${vars.LYNTTY_WEBAPP_URL}"`);
    lines.push(`export LYNTTY_HOME_DIR="${vars.LYNTTY_HOME_DIR}"`);
    lines.push(`export LYNTTY_PROJECT_DIR="${vars.LYNTTY_PROJECT_DIR}"`);
    lines.push(`export LYNTTY_VARIANT=dev`);
    lines.push(`export DEBUG=1`);
    lines.push(`export PATH="${path.join(envDir, "bin")}:$PATH"`);
    lines.push("");
    lines.push("# Commands exposed by this env");
    lines.push("# - lyntty");
    lines.push("# - lyntty-agent");
    lines.push("");
...
function writeEnvCommands(envDir: string): void {
    ...
    const commands = [
        {
            name: "lyntty",
            entrypoint: path.join(REPO_ROOT, "packages", "lyntty-cli", "bin", "lyntty.mjs"),
        },
        {
            name: "lyntty-agent",
            entrypoint: path.join(REPO_ROOT, "packages", "lyntty-agent", "bin", "lyntty-agent.mjs"),
        },
    ];
...
function buildCliCommand(envDir: string): string {
    return `source "${path.join(envDir, "env.sh")}" && lyntty`;
}
...
async function commandUp(template: Template, opts?: { noSwitch?: boolean }) {
    ...
    if (template === "authenticated-empty") {
        ...
        const buildResult = spawnSync("pnpm", ["build"], {
            cwd: path.join(REPO_ROOT, "packages", "lyntty-cli"),
            env: mergedEnv,
            stdio: "inherit",
        });
...
async function main(): Promise<void> {
    ...
    switch (subcommand) {
        ...
        case "tailscale":
            ...
            break;
        default:
            console.log(`Lyntty Environment Manager
