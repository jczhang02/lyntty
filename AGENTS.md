...
- Agents may proactively invoke `release-notes` after one exact existing product Release is resolved. Explicit `/skill:release-notes <version> <CodeName> <emoji> <channel-or-tag>` values override inference; otherwise derive the version and target only from unambiguous verified release identity, and infer CodeName and emoji from the dominant verified user-visible theme or the skill's channel fallback. Drafting never authorizes a public edit.
