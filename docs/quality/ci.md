...
| Job | Command | Purpose |
| --- | --- | --- |
| `repo-hygiene` | frozen root/docs installs + root lifecycle trust + docs lifecycle-script suppression/inventory + docs audit + hardening/release tests + `docs:check` + `docs:build` + Git whitespace check | lifecycle trust, docs dependency advisories, workflow/evidence/release contracts, complete docs links/anchors/locales/static export, and diff hygiene |
...
Every package job installs with `bun install --frozen-lockfile`. `repo-hygiene` installs the independent `docs/.site` lockfile with `--ignore-scripts`, inventories lifecycle-script trust, audits that graph, and runs the complete Fumadocs check and static build on every PR, even when no Markdown path changed. The separate `wire` job runs the root dependency audit. This keeps the existing required contexts authoritative for both dependency graphs, generated routes, locale counterparts, internal links, anchors, the Pages base path, and the global 404.
...
| `.github/workflows/docs.yml` | docs or published root-document changes on main + manual | docs | Installs docs dependencies with lifecycle scripts disabled, then checks/builds Fumadocs under `contents: read`. A separate checkout-free deploy job alone receives `pages: write` and OIDC permission. Root SECURITY, PRIVACY, and CONTRIBUTING language pairs are watched because the manifest publishes them. |
...
- `bun install --frozen-lockfile` — prove the root lockfile is complete.
- `bun pm untrusted` — inventory root lifecycle-script trust; it must report zero blocked scripts.
- `cd docs/.site && bun install --frozen-lockfile --ignore-scripts` — install the independent docs lockfile without executing lifecycle scripts.
