...
Required `Lyntty CI / Repo hygiene` 还会用 `bun install --frozen-lockfile --ignore-scripts` 安装独立的 `docs/.site` lockfile，禁止安装阶段执行 lifecycle script，并在每个 PR 上运行：
...
`.github/workflows/docs.yml` 的自动部署只由 main 上的匹配 push 触发，同时保留 `workflow_dispatch` 手动部署。Build job 同样以 `--ignore-scripts` 安装 docs 依赖，并且只持有 `contents: read`；不 checkout、也不执行项目依赖的 deploy job 才持有 `pages: write` 与 OIDC 权限。除 `docs/**` 外，workflow 还监听站点 manifest 发布的根级 SECURITY、PRIVACY 和 CONTRIBUTING 双语文件。域名和 `/lyntty` base path 不变。
...
- `bun install --frozen-lockfile` 与 `bun pm untrusted`：证明 root lockfile 完整并盘点 lifecycle script trust；root 与 docs audit 分别覆盖两个独立 lockfile。
- `cd docs/.site && bun install --frozen-lockfile --ignore-scripts`：按独立 docs lockfile 安装，同时禁止安装阶段执行 lifecycle script。
