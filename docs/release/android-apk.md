...
- Stable Promotion treats physical Android validation as optional. If `physical_phone_accepted=true`, the exact Compatibility Candidate APK must have passed that validation and its SHA-256 is required. If false, the accepted hash must be empty; no waiver phrase or Release-body warning is required. Both modes publish a checksummed and attested schema-2 `android-validation.json` that records the selected mode and exact APK digest.
