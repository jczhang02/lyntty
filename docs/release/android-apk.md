...
3. The normal path requires the operator to upgrade and fresh-start test the exact Candidate on physical Android. If that matrix cannot be run, publication requires a separate protected policy PR, an explicit owner authorization, `physical_phone_accepted=false`, and the exact dispatch phrase `I accept publishing this exact Candidate without physical Android validation`. The workflow rejects an empty waiver, simultaneous physical acceptance and waiver, or any other phrase.
4. The exceptional policy PR is itself part of the exact Candidate-to-Promotion delta and may change only the Promotion workflow, its hardening test, this English/Chinese runbook, and the existing R86 evidence paths. It cannot change or rebuild the APK, Candidate manifest, five release assets, provenance, attestations, allowlist binding, tag, title, or version.
