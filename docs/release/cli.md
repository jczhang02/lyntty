...
Channel discovery, signed Compatibility BOM verification, SBOM/provenance, macOS notarization, Windows Authenticode, and publication are separate release gates. Until those gates are connected, `update apply` intentionally requires an explicit trusted manifest digest and does not auto-discover a channel.
