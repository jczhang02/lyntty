...
- **Surface** — 控制入口，例如 native pi TUI、Android app 或 debug web console。
...
- **Sessions Home** — recent、running、waiting、failed、completed sessions 的 daily entry。
- **Session Remote** — 单个 active session/runtime 的 cockpit。
- **Review Evidence** — 根据 evidence 判断已完成或进行中工作的 mode。
- **Node Management / Pairing** — paired computers、QR/code pairing、health、trust、heartbeat、roots 和 diagnostics。
- **Settings / Recovery** — relay URL、owner/device binding、revocation 和 diagnostics。
