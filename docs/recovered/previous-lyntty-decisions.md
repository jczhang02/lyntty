...
## Home Hierarchy Decision

Earlier grill prep favored paired nodes first and an attention-first inbox. Current product direction uses session-first daily navigation instead:
...
- Session/runtime is the current protocol truth and the daily object user wants to resume or supervise.
- Node/computer still matters for pairing, trust, diagnostics, and choosing where new work runs.
- Attention state belongs on session rows, not in a separate product object.
- Task labels can be derived later from session name/goal.
- Chat/composer is a session action, not the whole product.
...
## Sessions Home Decision

Sessions Home answers: what work can I resume, unblock, or inspect now?
...
5. node filter, new session, and node-management actions.
...
- runtime running: default to queued follow-up;
- `steer` is an explicit interrupt action;
...
- Full E2E encryption is not part of current product scope.
...
Telegram, ntfy, Discord, and Web Push are not part of current product scope.
...
- Web/PWA was not acceptance for that dogfood gate.

## ADR 0001 Summary

Recovered ADR decision:

- `apps/client/` was a bounded dogfood control client, not product web/PWA client.
...
- real-alpha gate completed as private technical gate.
- dogfood-local gate completed.
- HeroUI Native dogfood gate passed for emulator/public-preview/native-smoke scope on 2026-06-09.
- Physical phone not run because no physical Android device was attached.
- Old git state: branch `main` was 16 commits ahead of old `origin/main`, with many uncommitted dogfood files.
...
The product PRD now follows the narrower direction from these recovered decisions:

- Android-first product surface, not product PWA/web.
- Debug web is developer tooling only.
- `pi` runtime is primary; adapters for Codex/OpenCode are future extension points.
- Sessions Home is the daily entry; Node Management is secondary.
