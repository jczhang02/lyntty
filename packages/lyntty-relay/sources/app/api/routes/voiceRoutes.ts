...
import { VoiceConversationResponseSchema, VoiceUsageResponseSchema } from "lyntty-wire";
...
function deriveElevenUserId(lynttyUserId: string): string {
    const hmac = crypto.createHmac("sha256", process.env.HANDY_MASTER_SECRET!);
    hmac.update(lynttyUserId);
