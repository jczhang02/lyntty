...
## Setup
- 2 lyntty-relay replicas behind a `LoadBalancer` service via `minikube tunnel`
...
**Observation:** `kubectl exec lyntty-redis-0 -- redis-cli XINFO STREAM socket.io`
...
1. **Re-register on every reconnect** (client side, daemon)
    - Already done by lyntty-cli for fresh connects but the
