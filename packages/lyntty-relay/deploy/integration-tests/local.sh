...
echo "=== Lyntty Local Deployment (minikube) ==="
...
echo "Building lyntty-relay:local image..."
docker build -t lyntty-relay:local -f "$REPO_ROOT/Dockerfile.server" "$REPO_ROOT"
...
kubectl wait --for=condition=ready pod -l app=lyntty-postgres --timeout=60s

# Run migrations via a one-shot job
kubectl run lyntty-migrate --rm -i --restart=Never \
    --image=lyntty-relay:local \
    --image-pull-policy=Never \
    --env="DATABASE_URL=postgresql://lyntty:lyntty@lyntty-postgres:5432/lyntty" \
    -- sh -c "cd /repo && npx prisma migrate deploy --schema=packages/lyntty-relay/prisma/schema.prisma" \
    2>/dev/null || true
