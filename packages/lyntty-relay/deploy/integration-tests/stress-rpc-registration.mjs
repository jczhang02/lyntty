...
// 1. Machine daemon connects + registers spawn-lyntty-session (fire-and-forget)
// 2. "iOS" caller immediately calls spawn-lyntty-session via RPC
...
async function iosSessionFlow() {
    ...
    for (let round = 0; round < ROUNDS; round++) {
        ...
        const MACHINE_METHOD = `${machineId}:spawn-lyntty-session`;
