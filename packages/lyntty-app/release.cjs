...
function printAvailableOptions() {
  console.error("Available lyntty-app release options:");
...
async function promptForAction() {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    console.error("Interactive lyntty-app release selection requires a TTY.");
  ...
  try {
    ...
    console.error(
      "Run `pnpm install` from repository root, then run `pnpm --filter lyntty-app release` again."
    );
    process.exit(1);
  }

  const actionId = await select({
    message: "What should be released for lyntty-app?",
    pageSize: 10,
...
async function main() {
  ...
  if (input && !action) {
    console.error(`Unknown lyntty-app release option: ${input}`);
    printAvailableOptions();
    process.exit(1);
  }
  ...
  console.log(`Running lyntty-app release option: ${action.label}`);
