...
export type AgentFlavor = 'pi' | 'claude' | 'codex' | 'gemini' | string | null | undefined;
...
export function getAvailablePermissionModes(
    ...
    if (flavor === 'claude' || flavor === 'codex' || flavor === 'openclaw' || flavor === 'pi') {
...
export function getHardcodedEffortLevels(flavor: AgentFlavor): EffortLevel[] {
    ...
    if (flavor === 'pi') return getCodexEffortLevels();
    return [];
}
