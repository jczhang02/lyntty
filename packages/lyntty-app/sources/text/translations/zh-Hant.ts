...
export const zhHant: TranslationStructure = {
    ...
    settings: {
        ...
        audioAssistant: '語音助理',
        audioAssistantSubtitle: '設定語音互動偏好',
        featuresTitle: '功能',
        featuresSubtitle: '啟用或停用應用程式功能',
        developer: '開發者',
        developerTools: '開發者工具',
        about: '關於',
        aboutFooter: 'Lyntty 是一個 pi 和 pi 行動用戶端。它採用端對端加密，您的帳戶僅儲存在本機裝置上。與 Anthropic 無關聯。',
        whatsNew: '更新日誌',
    ...
    settingsFeatures: {
        ...
        disableDiagnostics: '停用分析',
        diagnosticsDisabled: '所有追蹤和遙測已停用',
        diagnosticsEnabled: '匿名使用分析已啟用',
        imageUpload: '圖片上傳',
    ...
    errors: {
        ...
        audioSessionFailed: '啟動語音工作階段失敗',
        audioServiceUnavailable: '語音服務暫時無法使用',
        audioLimitReachedTitle: '已達語音上限',
        audioHardLimitReached: ({ hours }: { hours: number }) => `您本月已使用超過 ${hours} 小時的語音。這是允許的最大用量。您可以在語音設定中配置自己的 ElevenLabs 代理，以使用您自己的配額。`,
        audioConversationLimitReached: '您本月已達到語音對話的最大次數。我們未來可能會新增按需語音使用功能——如果您遇到此限制，請在 github.com/nicepkg/lyntty/issues 提交 issue。',
    ...
    sessionInfo: {
        ...
        failedToCopySessionId: '複製 Lyntty 工作階段 ID 失敗',
        lynttySessionId: 'Lyntty 工作階段 ID',
        ...
        aiProvider: 'AI 提供者',
        failedToCopypiCodeSessionId: '複製 pi 工作階段 ID 失敗',
        failedToCopypiThreadId: '複製 pi 執行緒 ID 失敗',
        ...
        resumeTakeoverPrompt: '未偵測到控制此工作階段的活躍 Pi 擴充功能。請選擇 lynttyd 取得 activation lock 的方式；「等待」最安全。',
        resumeWait: '等待',
        resumeTakeOver: '接管',
        resumeStop: '停止並恢復',
        ...
        updateCliInstructions: '請執行 npm install -g lyntty@latest',
    ...
    settingsAudio: {
        ...
        byoTitle: '使用自己的代理',
        byoDescription: '使用您自己的 ElevenLabs 代理取代 Lyntty 預設代理。無需訂閱 — 直接使用您自己的 ElevenLabs 帳戶連線。您的代理必須定義兩個用戶端工具：messagepiCode（向編碼代理傳送文字）和 processPermissionRequest（允許或拒絕工具使用）。透過 {{initialConversationContext}} 動態變數接收工作階段上下文。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未設定',
        customAgentIdDescription: '輸入您的 ElevenLabs Agent ID。留空則使用 Lyntty 預設代理。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接連線',
        bypassTokenSubtitle: '跳過 Lyntty 伺服器，直接連線到 ElevenLabs',
        promptGuideTitle: '代理提示詞指南',
        promptGuideDescription: '您的 ElevenLabs 代理需要：\n\n• 工具：messagepiCode — 參數：message (string)。向活躍的編碼工作階段傳送訊息。\n• 工具：processPermissionRequest — 參數：decision ("allow" 或 "deny")。核准或拒絕待處理的工具權限。\n• 動態變數：{{initialConversationContext}} — 啟動時接收工作階段歷史和上下文。\n\n代理充當使用者和編碼代理之間的語音橋梁。它應該簡潔，僅在被呼叫時回應，並在編碼代理完成工作時進行報告。',
    ...
    settingsAccount: {
        ...
        diagnostics: '分析',
        diagnosticsDisabled: '不分享資料',
        diagnosticsEnabled: '分享匿名使用資料',
        dangerZone: '危險區域',
        logout: '登出',
    ...
    machine: {
        ...
        offlineHelp: '• 確保您的電腦在線上\n• 執行 `lyntty daemon status` 進行診斷\n• 您是否在執行最新的 CLI 版本？請使用 `npm install -g lyntty@latest` 升級',
        daemon: '守護程序',
        status: '狀態',
        stopDaemon: '停止守護程序',
    ...
    appWide: {
        ...
        connect: "連接",
        connectANewDeviceViaWeb: "通過 Web 連接新設备",
        connectDevice: "連接設备",
        connecting: "連接中...",
        ...
        currentTokenValue: ({ value0 }: { value0: string | number }) => `当前 token ${value0}`,
        daemonStopped: "Daemon 已停止",
        dangerouslySkipPermissions: "危险地跳過權限",
        deleted: "已刪除",
        deletePushToken: "刪除推送 Token",
        details: "詳情",
        diff: "Diff",
        done: "完成",
        ...
        failedToDeleteArtifact: "刪除 artifact 失败",
        failedToDeletePushToken: "刪除推送 token 失败。",
        failedToLoadPushNotificationSettings: "加載推送通知設置失败。",
        failedToOpenTextSelectionPleaseTryAgain: "打開文本選擇失败。請重試。",
        failedToRefreshThisDevicePushToken: "刷新此設备推送 token 失败。",
        ...
        fastEfficient: "快速且高效",
        fastest: "最快",
        fetchesTheCurrentExpoTokenAndRegistersItAgain: "获取当前 Expo token 并重新注册。",
        fileDiffsSidebar: "文件 Diff 侧边栏",
        fullSessionObject: "完整會話对象",
        ...
        lynttySessionId: "Lyntty 會話 ID",
        lynttySessionIdCopied: "Lyntty 會話 ID 已復制",
        ...
        noRegisteredPushTokens: "没有已注册的推送 token",
        noResults: "无结果",
        noUsageDataAvailable: "没有可用使用量數據",
        noWorktree: "无 worktree",
        onceThisDeviceIsRegisteredItWillAppearHere: "此設备注册后會顯示在這里。",
        ...
        pleaseAllowMicrophoneAccessInYourBrowserSettingsYou: "請在瀏覽器設置中允许麦克风访問。你可能需要點擊地址栏中的锁圖標，并為此网站啟用麦克风權限。",
        ...
        pushNotificationsAreNotEnabledForThisDeviceYet: "此設备尚未啟用推送通知。",
        rawJsonDevMode: "原始 JSON（開發模式）",
        recent: "最近",
        registeredTokensValue: ({ value0 }: { value0: string | number }) => `已注册 Token（${value0}）`,
        relaySessionsUpTo5MostRecent: "Relay 會話（最多最近 5 個）",
        ...
        removeValueFromYourAccount: ({ value0 }: { value0: string | number }) => `從帳戶中移除 ${value0}？`,
        renameMachine: "重命名機器",
        requestPermissionAgain: "再次請求權限",
        reRegisterThisDevice: "重新注册此設备",
        ...
        showsEveryPushTokenRegisteredOnYourAccountTap: "顯示帳戶上注册的所有推送 token。點按舊 token 可刪除。",
        ...
        thisDevice: "此設备",
        thisDevicePushTokenWasRefreshed: "此設备推送 token 已刷新。",
