...
export const zhHans: TranslationStructure = {
    ...
    settings: {
        ...
        audioAssistant: '语音助手',
        audioAssistantSubtitle: '配置语音交互偏好',
        featuresTitle: '功能',
        featuresSubtitle: '启用或禁用应用功能',
        developer: '开发者',
        developerTools: '开发者工具',
        about: '关于',
        aboutFooter: 'Lyntty 是一个 pi 和 pi 移动客户端。它采用端到端加密，您的账户仅存储在本地设备上。与 Anthropic 无关联。',
        whatsNew: '更新日志',
    ...
    settingsFeatures: {
        ...
        disableDiagnostics: '禁用分析',
        diagnosticsDisabled: '所有跟踪和遥测已禁用',
        diagnosticsEnabled: '匿名使用分析已启用',
        imageUpload: '图片上传',
    ...
    errors: {
        ...
        audioSessionFailed: '启动语音会话失败',
        audioServiceUnavailable: '语音服务暂时不可用',
        audioLimitReachedTitle: '已达语音上限',
        audioHardLimitReached: ({ hours }: { hours: number }) => `您本月已使用超过 ${hours} 小时的语音。这是允许的最大用量。您可以在语音设置中配置自己的 ElevenLabs 代理，以使用您自己的配额。`,
        audioConversationLimitReached: '您本月已达到语音对话的最大次数。我们未来可能会添加按需语音使用功能——如果您遇到此限制，请在 github.com/nicepkg/lyntty/issues 提交 issue。',
    ...
    sessionInfo: {
        ...
        failedToCopySessionId: '复制 Lyntty 会话 ID 失败',
        lynttySessionId: 'Lyntty 会话 ID',
        ...
        aiProvider: 'AI 提供商',
        failedToCopypiCodeSessionId: '复制 pi 会话 ID 失败',
        failedToCopypiThreadId: '复制 pi 线程 ID 失败',
        ...
        resumeTakeoverPrompt: '未检测到控制此会话的活跃 Pi 扩展。请选择 lynttyd 获取 activation lock 的方式；“等待”最安全。',
        resumeWait: '等待',
        resumeTakeOver: '接管',
        resumeStop: '停止并恢复',
        ...
        updateCliInstructions: '请运行 npm install -g lyntty@latest',
        deleteSession: '删除会话',
    ...
    settingsAudio: {
        ...
        byoTitle: '使用自己的代理',
        byoDescription: '使用您自己的 ElevenLabs 代理代替 Lyntty 默认代理。无需订阅 — 直接使用您自己的 ElevenLabs 账户连接。您的代理必须定义两个客户端工具：messagepiCode（向编码代理发送文本）和 processPermissionRequest（允许或拒绝工具使用）。通过 {{initialConversationContext}} 动态变量接收会话上下文。',
        customAgentId: 'ElevenLabs Agent ID',
        customAgentIdNotSet: '未配置',
        customAgentIdDescription: '输入您的 ElevenLabs Agent ID。留空则使用 Lyntty 默认代理。',
        customAgentIdPlaceholder: 'e.g. abc123def456',
        bypassToken: '直接连接',
        bypassTokenSubtitle: '跳过 Lyntty 服务器，直接连接到 ElevenLabs',
        promptGuideTitle: '代理提示词指南',
        promptGuideDescription: '您的 ElevenLabs 代理需要：\n\n• 工具：messagepiCode — 参数：message (string)。向活跃的编码会话发送消息。\n• 工具：processPermissionRequest — 参数：decision ("allow" 或 "deny")。批准或拒绝待处理的工具权限。\n• 动态变量：{{initialConversationContext}} — 启动时接收会话历史和上下文。\n\n代理充当用户和编码代理之间的语音桥梁。它应该简洁，仅在被呼叫时回应，并在编码代理完成工作时进行报告。',
    ...
    settingsAccount: {
        ...
        diagnostics: '分析',
        diagnosticsDisabled: '不分享数据',
        diagnosticsEnabled: '分享匿名使用数据',
        dangerZone: '危险区域',
        logout: '登出',
    ...
    machine: {
        ...
        offlineHelp: '• 确保您的计算机在线\n• 运行 `lyntty daemon status` 进行诊断\n• 您是否在运行最新的 CLI 版本？请使用 `npm install -g lyntty@latest` 升级',
        daemon: '守护进程',
        status: '状态',
        stopDaemon: '停止守护进程',
    ...
    appWide: {
        ...
        connect: "连接",
        connectANewDeviceViaWeb: "通过 Web 连接新设备",
        connectDevice: "连接设备",
        connecting: "连接中...",
        ...
        currentDeviceMetadataComesFromThisPhoneOlderTokens: "当前设备元数据来自这台手机。旧 token 使用 token 指纹和服务器时间戳。",
        currentTokenValue: ({ value0 }: { value0: string | number }) => `当前 token ${value0}`,
        daemonStopped: "Daemon 已停止",
        dangerouslySkipPermissions: "危险地跳过权限",
        deleted: "已删除",
        deletePushToken: "删除推送 Token",
        details: "详情",
        diff: "Diff",
        done: "完成",
        ...
        failedToDeleteArtifact: "删除 artifact 失败",
        failedToDeletePushToken: "删除推送 token 失败。",
        failedToLoadPushNotificationSettings: "加载推送通知设置失败。",
        failedToOpenTextSelectionPleaseTryAgain: "打开文本选择失败。请重试。",
        failedToRefreshThisDevicePushToken: "刷新此设备推送 token 失败。",
        ...
        lynttySessionId: "Lyntty 会话 ID",
        lynttySessionIdCopied: "Lyntty 会话 ID 已复制",
        ...
        onceThisDeviceIsRegisteredItWillAppearHere: "此设备注册后会显示在这里。",
        ...
        pleaseAllowMicrophoneAccessInYourBrowserSettingsYou: "请在浏览器设置中允许麦克风访问。你可能需要点击地址栏中的锁图标，并为此网站启用麦克风权限。",
        ...
        pushNotificationsAreNotEnabledForThisDeviceYet: "此设备尚未启用推送通知。",
        rawJsonDevMode: "原始 JSON（开发模式）",
        recent: "最近",
        registeredTokensValue: ({ value0 }: { value0: string | number }) => `已注册 Token（${value0}）`,
        relaySessionsUpTo5MostRecent: "Relay 会话（最多最近 5 个）",
        ...
        removeValueFromYourAccount: ({ value0 }: { value0: string | number }) => `从账户中移除 ${value0}？`,
        renameMachine: "重命名机器",
        requestPermissionAgain: "再次请求权限",
        reRegisterThisDevice: "重新注册此设备",
        ...
        showsEveryPushTokenRegisteredOnYourAccountTap: "显示账户上注册的所有推送 token。点按旧 token 可删除。",
        ...
        thisDevice: "此设备",
        thisDevicePushTokenWasRefreshed: "此设备推送 token 已刷新。",
