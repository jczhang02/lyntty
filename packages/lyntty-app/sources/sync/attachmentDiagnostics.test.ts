...
describe('sanitizeAttachmentUrlHost', () => {
    it('keeps only the host and port from absolute URLs', () => {
        expect(sanitizeAttachmentUrlHost('https://files.cluster-fluster.com/lyntty/sessions/ref?X-Amz-Signature=secret'))
...
describe('classifyAttachmentTransferTarget', () => {
    it('classifies URLs on the Lyntty API host as lyntty-api', () => {
    ...
    it('classifies other valid hosts as external-storage', () => {
        expect(classifyAttachmentTransferTarget(
            'https://files.cluster-fluster.com/lyntty/abc?policy=secret',
            'https://api.cluster-fluster.com',
...
describe('attachment diagnostic serialization', () => {
    it('builds a safe diagnostic from a response', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-upload',
            method: 'POST',
            url: 'https://files.cluster-fluster.com/lyntty/sessions/ref?X-Amz-Signature=secret&policy=secret',
        ...
        expect(serialized).not.toContain('/lyntty/sessions/ref');
    ...
    it('keeps network messages but not source URLs', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            method: 'GET',
            url: 'https://files.cluster-fluster.com/lyntty/sessions/ref?AWSAccessKeyId=secret',
            serverUrl: 'https://api.cluster-fluster.com',
    ...
    it('does not emit dirty URL hosts from raw diagnostics', () => {
        ...
        const safeDiagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            url: 'https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret',
            serverUrl: 'https://api.cluster-fluster.com',
        });
    ...
    it('sanitizes unsafe messages and reasons before serialization', () => {
        const diagnostic = createAttachmentDiagnostic({
            ...
            url: 'https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret',
            serverUrl: 'https://api.cluster-fluster.com',
            message: 'Fetch failed for https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret&policy=secret using Bearer secret-token',
            reason: 'Read failed from file:///Users/devdvlive/Projects/lyntty/local.bin and /Users/devdvlive/Projects/lyntty/other.bin',
        });
        ...
        expect(serialized).not.toContain('/lyntty/ref');
    ...
    it('redacts non-http absolute URLs in diagnostic text', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'blob-download',
            method: 'GET',
            message: 'Fetch failed for s3://bucket/lyntty/ref?X-Amz-Signature=secret',
    ...
    it('redacts desktop local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Failed to fetch',
            reason: 'Read failed from /Users/devdvlive/My Projects/lyntty/blob.bin',
        });
        ...
        expect(serialized).not.toContain('lyntty/blob.bin');
        expect(serialized).not.toContain('blob.bin');
    });

    it('redacts file URI local paths with spaces in path segments', () => {
        const source = 'Read failed from file:///Users/devdvlive/My Projects/lyntty/blob.bin';
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: source,
        });
        ...
        for (const rendered of [serialized, message]) {
            ...
            expect(rendered).not.toContain('Projects/lyntty');
    ...
    it('redacts Windows desktop local paths with spaces in path segments', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: 'Read failed from C:\\Users\\devdvlive\\My Projects\\lyntty\\blob.bin',
        });
        ...
        expect(serialized).not.toContain('My Projects');
        expect(serialized).not.toContain('Projects\\lyntty');
        expect(serialized).not.toContain('blob.bin');
    });
    ...
    it('redacts assignment-prefixed local paths before diagnostic serialization', () => {
        const sources = [
            ...
            'path=/data/user/0/com.lyntty/cache/blob.bin',
        ...
        for (const source of sources) {
            ...
            expect(rendered).not.toContain('C:/Users');
            expect(rendered).not.toContain('My Projects');
            expect(rendered).not.toContain('com.lyntty');
            expect(rendered).not.toContain('blob.bin');
        }
    });
    ...
    it('redacts mobile local paths in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Read failed from /data/user/0/com.lyntty/cache/blob.bin',
    ...
    it('redacts Android app data paths in reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            reason: 'Read failed from /data/data/com.lyntty/cache/blob.bin',
        });

        const serialized = JSON.stringify(diagnostic);
        expect(serialized).toContain('[local-file]');
        expect(serialized).not.toContain('/data/data');
        expect(serialized).not.toContain('com.lyntty');
        expect(serialized).not.toContain('blob.bin');
    });
    ...
    it('sanitizes dirty whitelisted fields when formatting diagnostics for logs', () => {
        const rendered = formatAttachmentDiagnosticForLog({
            leg: 'blob-download',
            method: 'GET',
            host: 'https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret',
            target: 'external-storage',
            status: 403,
            statusText: 'Forbidden from https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret',
            message: 'Failed to fetch',
            reason: 'missing-blob-key',
        }, {
            platform: 'web lyntty/session-1/ref',
            client: 'web/1.2.3 Bearer secret-token /data/data/com.lyntty/cache/blob.bin',
        });
        ...
        expect(serialized).not.toContain('com.lyntty');
        expect(serialized).not.toContain('blob.bin');
        expect(serialized).not.toContain('lyntty/session-1/ref');
    });
    ...
    it('drops dirty non-host values from host output', () => {
        const rendered = [
            'C:\\Users\\devdvlive\\My Projects\\lyntty\\blob.bin',
            'data:,SECRET',
            'user:secret@files.cluster-fluster.com',
            'file:///Users/devdvlive/My Projects/lyntty/blob.bin',
    ...
    it('redacts bare attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'Render failed for ref: lyntty/session-1/ref and ref lyntty/session-1/ref',
    ...
    it('redacts standalone slash-bearing attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'lyntty/session-1/ref',
            reason: 'missing-blob-key lyntty/session-1/ref',
        });
    ...
    it('redacts leading-slash attachment refs in messages and reasons', () => {
        const diagnostic = createAttachmentDiagnostic({
            leg: 'decrypt-render',
            message: 'missing /lyntty/session-1/ref?X-Amz-Signature=secret',
            reason: 'ref=/lyntty/session-1/ref',
        });

        const serialized = JSON.stringify(diagnostic);
        expect(serialized).toContain('[attachment-ref]');
        expect(serialized).not.toContain('/lyntty/session-1/ref');
        expect(serialized).not.toContain('ref=/lyntty/session-1/ref');
        expect(serialized).not.toContain('X-Amz-Signature');
    });
...
describe('AttachmentDiagnosticError', () => {
    ...
    it('sanitizes unsafe wrapper error messages', () => {
        const error = createAttachmentDiagnosticError(
            'Blob upload failed for https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret using Bearer secret-token at /data/user/0/com.lyntty/cache/blob.bin',
            {
                leg: 'blob-upload',
                method: 'POST',
                url: 'https://files.cluster-fluster.com/lyntty/ref?X-Amz-Signature=secret',
