...
// Resolve lyntty home dir — same pattern as lyntty-cli/src/configuration.ts
let lynttyHome: string;
if (process.env.LYNTTY_HOME_DIR) {
    lynttyHome = process.env.LYNTTY_HOME_DIR.replace(/^~/, homedir());
} else {
    lynttyHome = join(homedir(), '.lyntty');
}

const logsDir = join(lynttyHome, 'app-logs');
