...
import { lynttyClient, useLynttyState } from '@/lyntty/client'
...
function LynttyAuthPanel() {
    const state = useLynttyState()
    ...
    if (state.status === 'starting') {
        return <small className="plugin-detail__help">Loading Lyntty state…</small>
    }
            ...
            <div className="plugin-detail__row">
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('logout', () => lynttyClient.logout())}
                        disabled={busy !== null}
                    >
                        Disconnect
                    </button>
    ...
    if (state.status === 'authenticating' && authUrl) {
        return (
            <div className="plugin-detail__row plugin-detail__row--column">
                <div className="lyntty-auth__qr-wrap">
                    {qrDataUrl ? (
                        <img className="lyntty-auth__qr" src={qrDataUrl} alt="Lyntty authentication QR code" />
                    ) : (
                        <div className="lyntty-auth__qr lyntty-auth__qr--empty" />
                    )}
                </div>
                <div className="lyntty-auth__url-row">
                    <input
                        className="plugin-detail__input lyntty-auth__url"
                        value={authUrl}
                        readOnly
                        spellCheck={false}
                    />
                ...
                <div className="plugin-detail__actions">
                    <button
                        type="button"
                        className="plugins-page__action"
                        onClick={() => run('cancel', () => lynttyClient.cancelAuth())}
                        disabled={busy !== null}
                    >
                        Cancel
                    </button>
        ...
        <div className="plugin-detail__row plugin-detail__row--column">
            ...
            <div className="plugin-detail__actions">
                <button
                    type="button"
                    className="plugins-page__action plugins-page__action--primary"
                    onClick={() => run('create', () => lynttyClient.createAccount())}
                    disabled={busy !== null}
                >
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('link', () => lynttyClient.startLinkDevice())}
                    disabled={busy !== null}
                >
            ...
            <div className="lyntty-auth__url-row">
                ...
                <button
                    type="button"
                    className="plugins-page__action"
                    onClick={() => run('restore', () => lynttyClient.restoreSecret(secretKey))}
