...
/* ─────── Chats persistence (jotai store <-> <Lyntty home>/state.sqlite) ─────── */
...
const lyntty = {
    getState: (): Promise<LynttyStateSnapshot> =>
        ipcRenderer.invoke('lyntty:state:get'),
    createAccount: (): Promise<LynttyStateSnapshot> =>
        ipcRenderer.invoke('lyntty:create-account'),
    startLinkDevice: (): Promise<LynttyStateSnapshot> =>
        ipcRenderer.invoke('lyntty:start-link-device'),
    restoreSecret: (secretKey: string): Promise<LynttyStateSnapshot> =>
        ipcRenderer.invoke('lyntty:restore-secret', secretKey),
    cancelAuth: (): Promise<LynttyStateSnapshot> =>
        ipcRenderer.invoke('lyntty:cancel-auth'),
    logout: (): Promise<LynttyStateSnapshot> =>
        ipcRenderer.invoke('lyntty:logout'),
    clientStatus: (): Promise<LynttyAuthenticatedClientStatus> =>
        ipcRenderer.invoke('lyntty:client-status'),
    onState(cb: (state: LynttyStateSnapshot) => void) {
        const listener = (_: unknown, state: LynttyStateSnapshot) => cb(state)
        ipcRenderer.on('lyntty:state', listener)
        return () => ipcRenderer.off('lyntty:state', listener)
    },
}
...
if (process.contextIsolated) {
    try {
        ...
        contextBridge.exposeInMainWorld('lyntty', lyntty)
