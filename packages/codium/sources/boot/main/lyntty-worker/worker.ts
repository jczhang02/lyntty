...
if (!parentPort) {
    throw new Error('lyntty worker must be started via worker_threads')
}
...
const send = (m: LynttyWorkerMessage) => port.postMessage(m)

let credentials: StoredCredentials | null = null
let client: LynttyAuthenticatedClient | null = null
let linkFlow: LinkFlow | null = null
let state: LynttyStateSnapshot = {
    status: 'starting',
...
class LynttyAuthenticatedClient {
    private constructor(
        private readonly cfg: WorkerConfig,
        private readonly auth: StoredCredentials,
        private readonly encryption: LynttyEncryptionContext,
    ) {}

    static async create(
        cfg: WorkerConfig,
        auth: StoredCredentials,
    ): Promise<LynttyAuthenticatedClient> {
        const secret = decodeBase64(auth.secret, 'base64url')
        const encryption = await createEncryptionContext(secret)
        return new LynttyAuthenticatedClient(cfg, auth, encryption)
    }
    ...
    async request(path: string, init: RequestInit = {}): Promise<Response> {
        ...
        headers.set('X-Lyntty-Client', this.cfg.clientId)
        return fetch(`${this.cfg.serverUrl}${path}`, { ...init, headers })
    }
}

port.on('message', (msg: LynttyWorkerRequestWithId) => {
    Promise.resolve()
        .then(() => handle(msg))
        .catch((err) => {
...
async function boot(): Promise<void> {
    try {
        ...
        client = credentials ? await LynttyAuthenticatedClient.create(config, credentials) : null
        setState(snapshotForCredentials(credentials))
    } catch (err) {
        setError(`Failed to load Lyntty credentials: ${errString(err)}`)
    }
}

async function handle(msg: LynttyWorkerRequestWithId): Promise<void> {
...
async function startLinkDevice(requestId: string): Promise<void> {
    ...
    try {
        ...
        const flow: LinkFlow = {
            keypair,
            publicKey,
            authUrl: `lyntty:///account?${publicKey}`,
            startedAt: Date.now(),
            cancelled: false,
        }
        ...
        try {
            ...
            if (data.state === 'authorized') {
                if (typeof data.token !== 'string' || typeof data.response !== 'string') {
                    throw new Error('Lyntty auth response is missing token data')
                }
                const encrypted = decodeBase64(data.response)
                const secret = decryptBox(encrypted, flow.keypair.secretKey)
                if (!secret || secret.length !== 32) {
                    throw new Error('Failed to decrypt Lyntty account credentials')
                }
                linkFlow = null
            ...
            if (!flow.cancelled && linkFlow === flow) {
                linkFlow = null
                setError(`Lyntty link failed: ${errString(err)}`)
            }
            return
        }
        await delay(1000)
...
async function storeAuthenticated(token: string, secret: Uint8Array): Promise<void> {
    ...
    client = await LynttyAuthenticatedClient.create(config, credentials)
    setState(snapshotForCredentials(credentials))
}

async function createEncryptionContext(masterSecret: Uint8Array): Promise<LynttyEncryptionContext> {
    const contentSeed = await deriveKey(masterSecret, 'Lyntty EnCoder', ['content'])
    ...
    const anonId = encodeHex(
        (await deriveKey(masterSecret, 'Lyntty Coder', ['analytics', 'id'])).slice(0, 8),
    ).toLowerCase()
...
async function authGetToken(secret: Uint8Array): Promise<string> {
    ...
    if (typeof data.token !== 'string') {
        throw new Error('Lyntty authentication did not return a token')
    }
    return data.token
}
...
async function apiJson(path: string, body: unknown): Promise<Record<string, unknown>> {
    const response = await fetch(`${config.serverUrl}${path}`, {
        ...
        headers: {
            ...
            'X-Lyntty-Client': config.clientId,
        },
        body: JSON.stringify(body),
    })
    if (!response.ok) {
        const text = await response.text().catch(() => '')
        throw new Error(`Lyntty API ${response.status}: ${text || response.statusText}`)
    }
...
function snapshotForCredentials(auth: StoredCredentials | null): LynttyStateSnapshot {
    if (!auth) {
        return {
            ...stateBase(),
            status: 'unconfigured',
...
function setState(next: LynttyStateSnapshot): void {
    state = next
...
function respond(requestId: string, next: LynttyStateSnapshot, value?: unknown): void {
