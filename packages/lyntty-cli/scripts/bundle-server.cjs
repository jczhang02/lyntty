...
/**
 * Bundles lyntty-relay into a self-contained artifact directory.
 *
 * Uses `bun build --compile` to produce a single platform-specific binary, then copies the
 * pglite WASM/data files and prisma migrations alongside. lyntty-cli does NOT depend on the
 * lyntty-relay workspace package — we reach into the sibling directory at build time only.
 *
 * Layout produced:
 *   <out-dir>/
 *     <platform>/
 *       lyntty-relay                  # bun-compiled binary
...
const SERVER_DIR = path.resolve(REPO_ROOT, 'packages/lyntty-relay');
...
function platformBinaryName(plat) {
    return plat.endsWith('-win32') ? 'lyntty-relay.exe' : 'lyntty-relay';
}
...
function buildPlatform(plat, outRoot) {
    ...
    console.log(`\n→ Bundling lyntty-relay for ${plat} (${target})`);
    run(
        'bun',
        [
            'build',
...
function main() {
    ...
    console.log(`\n✓ lyntty-relay bundle written to ${outDir}`);
}
