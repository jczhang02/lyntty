...
/**
 * Runs `expo export -p web` in packages/lyntty-app and copies the output into
 * a package-owned artifact directory. By default this writes to lyntty-cli/tools/webapp
 * for local development. Release packaging can pass --out-dir to place it elsewhere.
 *
 * lyntty-cli does NOT depend on lyntty-app — we reach into the sibling at build time only.
 */
...
const APP_DIR = path.resolve(REPO_ROOT, 'packages/lyntty-app');
...
function main() {
    ...
    console.log(`→ Building lyntty-app web bundle (expo export)`);
    rmrf(APP_DIST);
