 ...
 *     │
 *     ├─► generateHookSettingsFile(port) ──► ~/.lyntty/tmp/hooks/session-hook-<pid>.json
 ...
 *
 * ### Triggered By
 * - `lyntty` (fresh start) - new session created
 * - `lyntty --continue` - continues last session (may fork)
 * - `lyntty --resume` - interactive picker, then resume
 * - `lyntty --resume <id>` - resume specific session
 * - `/compact` command - compacts and forks session
 * - Double-escape fork - user forks conversation in CLI
 *
 * ### Why Not Use File Watching?
 * File watching has race conditions when multiple Lyntty processes run.
 * With hooks, Claude directly tells THIS specific process about its session,
 * ensuring 1:1 mapping between Lyntty process and Claude session.
 */
