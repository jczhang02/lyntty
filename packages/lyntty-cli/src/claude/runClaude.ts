...
import { startLynttyServer } from '@/claude/utils/startLynttyServer';
...
export async function runClaude(credentials: Credentials, options: StartOptions = {}): Promise<void> {
    ...
    logger.debugLargeJson('[START] Lyntty process started', getEnvironmentInfo());
    logger.debug(`[START] Options: startedBy=${options.startedBy}, startingMode=${options.startingMode}`);

    // Validate daemon spawn requirements - fail fast on invalid config
    if (options.startedBy === 'daemon' && options.startingMode === 'local') {
        throw new Error('Daemon-spawned sessions cannot use local/interactive mode. Use --lyntty-starting-mode remote or spawn sessions directly from terminal.');
    }
    ...
    if (!machineId) {
        console.error(`[START] No machine ID found in settings, which is unexpected since authAndSetupMachineIfNeeded should have created it. Please report this issue on https://github.com/slopus/lyntty-cli/issues`);
        process.exit(1);
    }
    ...
    const forkedFromSessionId = process.env.LYNTTY_FORKED_FROM_SESSION_ID;
    const forkedFromMessageId = process.env.LYNTTY_FORKED_FROM_MESSAGE_ID;
    ...
    const reconnectSessionId = process.env.LYNTTY_RECONNECT_SESSION_ID;
    const reconnectKeyBase64 = process.env.LYNTTY_RECONNECT_ENCRYPTION_KEY;
    const reconnectVariant = process.env.LYNTTY_RECONNECT_ENCRYPTION_VARIANT as 'legacy' | 'dataKey' | undefined;
    const reconnectSeq = process.env.LYNTTY_RECONNECT_SEQ;
    const reconnectMetadataVersion = process.env.LYNTTY_RECONNECT_METADATA_VERSION;
    const reconnectAgentStateVersion = process.env.LYNTTY_RECONNECT_AGENT_STATE_VERSION;
    ...
    // Fork backfill: when this Lyntty session was just spawned as a fork
    // of another (LYNTTY_FORK_CLAUDE_SESSION_ID is set by the daemon at
    ...
    // historical messages back to the Lyntty client — so without an
    ...
    //
    // Skipped on reconnect (LYNTTY_RECONNECT_*) — that path reattaches
    // to the existing Lyntty session, where the server already has every
    // message it needs.
    const forkClaudeSessionId = process.env.LYNTTY_FORK_CLAUDE_SESSION_ID;
    if (!reconnectSessionId && forkClaudeSessionId) {
        ...
        try {
            ...
            // Bind the new Lyntty session to the forked Claude UUID up
    ...
    // alongside this Lyntty session. App-sent prompts also land in the
    ...
    // to the running Lyntty session. SDK-emitted assistant + tool_result
    ...
    // Start Lyntty MCP server
    const lynttyServer = await startLynttyServer(session);
    logger.debug(`[START] Lyntty MCP server started at ${lynttyServer.url}`);
    ...
    const exitCode = await loop({
        ...
        messageQueue,
        api,
        allowedTools: lynttyServer.toolNames.map(toolName => `mcp__lyntty__${toolName}`),
    ...
    // Stop Lyntty MCP server
    lynttyServer.stop();
    logger.debug('Stopped Lyntty MCP server');
