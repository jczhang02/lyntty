/**
 * Installation script for Lyntty daemon using macOS LaunchDaemons
 *
 * NOTE: This installation method is currently NOT USED in favor of auto-starting
 * the daemon when the user runs the lyntty command.
 *
 ...
 * 2. We assume users will run lyntty frequently (every time they open their laptop)
...
const PLIST_LABEL = 'com.lyntty-cli.daemon';
...
export async function install(): Promise<void> {
    try {
        ...
        // Get the path to the lyntty CLI executable
        const lynttyPath = process.argv[0]; // Node.js executable
        ...
        const plistContent = trimIdent(`
            ...
            <dict>
                ...
                <array>
                    <string>${lynttyPath}</string>
                    <string>${scriptPath}</string>
                    <string>lyntty-daemon</string>
                </array>

                <key>EnvironmentVariables</key>
                <dict>
                    <key>LYNTTY_DAEMON_MODE</key>
                    <string>true</string>
                </dict>
                ...
                <string>${os.homedir()}/.lyntty/daemon.err</string>

                <key>StandardOutPath</key>
                <string>${os.homedir()}/.lyntty/daemon.log</string>
        ...
        logger.info('Check logs at ~/.lyntty/daemon.log');
