...
export async function notifyDaemonSessionStarted(
  ...
  // app's resume-lyntty-session RPC fails with "not tracked by this daemon".
 ...
 * For instance we are not handling the case where we upgraded lyntty,
 ...
 * For instance when running `lyntty daemon status` we can show more information.
 */
 ...
 */
export async function isDaemonRunningCurrentlyInstalledLynttyVersion(): Promise<boolean> {
  ...
  // from the bundled version — e.g. when `lyntty-coder@0.13.1` was published as
