 ...
 *
 * This file boots one authenticated Lyntty environment and restarts the daemon
...
async function stopAllTrackedSessions(): Promise<void> {
  ...
  await Promise.all(
    sessions.map((session: any) =>
      stopDaemonSession(session.lynttySessionId ?? `PID-${session.pid}`).catch(() => false)
    ),
  );
}
...
describe('Daemon Integration Tests', { timeout: 180_000 }, () => {
  ...
  beforeEach(async () => {
    ...
    void spawnLynttyCLI(['daemon', 'start'], {
      stdio: 'ignore',
    });
  ...
  it('should track session-started webhook from terminal session', async () => {
    ...
    const mockMetadata: Metadata = {
      ...
      host: 'test-host',
      homeDir: '/test/home',
      lynttyHomeDir: '/test/lyntty-home',
      lynttyLibDir: '/test/lyntty-lib',
      lynttyToolsDir: '/test/lyntty-tools',
      hostPid: 99999,
      startedBy: 'terminal',
    ...
    expect(tracked.startedBy).toBe('lyntty directly - likely by user from terminal');
    expect(tracked.lynttySessionId).toBe('test-session-123');
  ...
  it('should spawn & stop a session via HTTP (not testing RPC route, but similar enough)', async () => {
    ...
    const spawnedSession = sessions.find(
      (s: any) => s.lynttySessionId === response.sessionId
    );
    ...
    expect(spawnedSession.lynttySessionId).toBeDefined();
    await stopDaemonSession(spawnedSession.lynttySessionId);
  });
  ...
  it('should track both daemon-spawned and terminal sessions', async () => {
    // Spawn a real lyntty process that looks like it was started from terminal
    const terminalLynttyProcess = spawnLynttyCLI([
    ...
    if (!terminalLynttyProcess || !terminalLynttyProcess.pid) {
      throw new Error('Failed to spawn terminal lyntty process');
    }
    ...
    const terminalSession = sessions.find(
      (s: any) => s.pid === terminalLynttyProcess.pid
    );
    const daemonSession = sessions.find(
      (s: any) => s.lynttySessionId === spawnResponse.sessionId
    );

    expect(terminalSession).toBeDefined();
    expect(terminalSession.startedBy).toBe('lyntty directly - likely by user from terminal');
    ...
    await stopDaemonSession(daemonSession.lynttySessionId);

    // Also kill the terminal process directly to be sure
    try {
      terminalLynttyProcess.kill('SIGTERM');
    } catch (e) {
  ...
  it('should update session metadata when webhook is called', async () => {
    ...
    const sessions = await listDaemonSessions();
    const session = sessions.find((s: any) => s.lynttySessionId === spawnResponse.sessionId);
  ...
  it('should handle concurrent session operations', async () => {
    ...
    const daemonSessions = sessions.filter(
      (s: any) => s.startedBy === 'daemon' && spawnedSessionIds.includes(s.lynttySessionId)
    );
    expect(daemonSessions.length).toBeGreaterThanOrEqual(3);

    // Stop all spawned sessions
    for (const session of daemonSessions) {
      expect(session.lynttySessionId).toBeDefined();
      await stopDaemonSession(session.lynttySessionId);
    }
  });
  ...
  it.skip('[skipped] should detect version mismatch and kill old daemon', { timeout: 100_000 }, async () => {
    ...
    try {
      ...
      await new Promise(resolve => setTimeout(resolve, parseInt(process.env.LYNTTY_DAEMON_HEARTBEAT_INTERVAL || '30000') + 10_000));
