...
export async function runCodex(opts: {
    ...
    try {
        ...
        console.error('  \x1b[36mlyntty claude\x1b[0m\n');
        process.exit(1);
    }
    ...
    // Start Lyntty MCP server (HTTP) and prepare STDIO bridge config for Codex
    ...
    const bridgeEntrypoint = join(projectPath(), 'bin', 'lyntty-mcp.mjs');
    const mcpServers = {
        lyntty: {
            command: process.execPath,
            args: ['--no-warnings', '--no-deprecation', bridgeEntrypoint, '--url', lynttyServer.url]
        }
    } as const;
    let first = true;
    ...
    try {
        ...
        const forkCodexThreadId = process.env.LYNTTY_FORK_CODEX_THREAD_ID;
        ...
        // Stop Lyntty MCP server
        logger.debug('[codex]: lynttyServer.stop');
        lynttyServer.stop();
