...
vi.mock('@/configuration', () => ({
    configuration: { lynttyHomeDir: '/home/test/.lyntty' },
}));
...
async function makeTempDir(): Promise<string> {
    const dir = await mkdtemp(join(tmpdir(), 'lyntty-codex-image-input-'));
    tempDirs.push(dir);
    return dir;
}
...
describe('resolveCodexImageCacheDir', () => {
    it('uses the explicit cache root when provided', () => {
        expect(resolveCodexImageCacheDir({
            cacheRootDir: '/tmp/lyntty-cache',
            sessionId: 'session-1',
        })).toBe('/tmp/lyntty-cache/session-1');
    });

    it('defaults to Lyntty local state instead of arbitrary OS temp', () => {
        expect(resolveCodexImageCacheDir({
            sessionId: 'session-4',
        })).toBe('/home/test/.lyntty/codex-image-cache/session-4');
    });

    it('keeps malformed session ids inside the cache root', () => {
        const cacheRootDir = '/tmp/lyntty-cache';
