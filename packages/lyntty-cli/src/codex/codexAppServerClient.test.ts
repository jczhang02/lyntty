...
describe('CodexAppServerClient sandbox integration', () => {
    ...
    it('sends extra localImage input items and omits empty text for image-only turns', async () => {
        ...
        await client.sendTurnAndWait('', {
            extraInputItems: [{ type: 'localImage', path: '/tmp/lyntty-image.png' }],
        });

        expect(requests.find((msg) => msg.method === 'turn/start')?.params).toMatchObject({
            threadId: 'thread-images',
            input: [{ type: 'localImage', path: '/tmp/lyntty-image.png' }],
        });
                    ...
                    setTimeout(() => {
                        ...
                        pushJsonLine(stdout, {
                            ...
                            params: {
                                ...
                                message: 'Allow the lyntty MCP server to run tool "change_title"?',
                                requestedSchema: {
                                    type: 'object',
                                    properties: {},
                                },
