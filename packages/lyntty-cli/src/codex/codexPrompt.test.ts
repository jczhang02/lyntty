...
describe('buildCodexTurnPrompt', () => {
    it('prepends Lyntty append system prompt before the first Codex user message', () => {
    ...
    it('does not inject Lyntty preamble on normal follow-up turns', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'continue',
            mode: {
                appendSystemPrompt: '<options><option>Yes</option></options>',
    ...
    it('can re-inject Lyntty append prompt without title instruction after a thread reset', () => {
        const prompt = buildCodexTurnPrompt({
            message: 'start fresh',
            mode: {
                appendSystemPrompt: '<options><option>Yes</option></options>',
