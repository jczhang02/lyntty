 ...
 * through Lyntty CLI. It manages the agent lifecycle, session state, and
 * communication with the Lyntty relay and mobile app.
 */
...
export async function runGemini(opts: {
  ...
  //
  // Fetch Gemini cloud token (from 'lyntty connect gemini')
  //
  ...
  try {
    ...
    if (vendorToken?.oauth?.access_token) {
      ...
      logger.debug('[Gemini] Using OAuth token from Lyntty cloud');
  ...
  // (assigned later after Lyntty relay setup)
  ...
  //
  // Start Lyntty MCP server and create Gemini backend
  //

  const lynttyServer = await startLynttyServer(session);
  const bridgeCommand = join(projectPath(), 'bin', 'lyntty-mcp.mjs');
  const mcpServers = {
    lyntty: {
      command: bridgeCommand,
      args: ['--url', lynttyServer.url]
    }
  };
    ...
    switch (msg.type) {
        ...
        if (msg.status === 'running') {
          ...
          if (errorMessage.includes('Authentication required')) {
            errorMessage = `Authentication required.\n` +
              `For Google Workspace accounts, run: lyntty gemini project set <project-id>\n` +
              `Or use a different Google account: lyntty connect gemini\n` +
      ...
      case 'tool-result':
        ...
        if (msg.toolName === 'change_title' ||
            ...
            msg.toolName === 'lyntty__change_title') {
    ...
    while (!shouldExit) {
      ...
      try {
        ...
        pendingChangeTitle = message.message.includes('change_title') ||
                             message.message.includes('lyntty__change_title');
          ...
          if (typeof error === 'object' && error !== null) {
            ...
            // Check for authentication error (Google Workspace accounts need project ID)
              ...
              errorMsg = `Authentication required. For Google Workspace accounts, you need to set a Google Cloud Project:\n` +
                         `  lyntty gemini project set <your-project-id>\n` +
                         `Or use a different Google account: lyntty connect gemini\n` +
                         `Guide: https://goo.gle/gemini-cli-auth-docs#workspace-gca`;
            }
