...
export async function runAcp(opts: {
  ...
  if (response) {
    logAcp('muted', `Lyntty Session ID: ${response.id}`);
  }
  ...
  const mcpServers = {
    lyntty: {
      command: join(projectPath(), 'bin', 'lyntty-mcp.mjs'),
  ...
  try {
    ...
    try {
      ...
      logger.debug(`[${opts.agentName}] Failed to stop Lyntty MCP server:`, error);
    }
