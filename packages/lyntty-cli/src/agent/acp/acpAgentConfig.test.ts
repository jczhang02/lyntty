...
describe('resolveAcpAgentConfig', () => {
  ...
  it('throws with helpful usage when no args are provided', () => {
    expect(() => resolveAcpAgentConfig([])).toThrow('Usage: lyntty acp <agent-name> or lyntty acp -- <command> [args]');
  });

  it('throws when separator form omits command', () => {
    expect(() => resolveAcpAgentConfig(['--'])).toThrow('Missing command after "--". Usage: lyntty acp -- <command> [args]');
  });
});
