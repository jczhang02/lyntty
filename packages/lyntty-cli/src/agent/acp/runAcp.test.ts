...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    mockProjectPath: vi.fn(() => '/tmp/lyntty'),
...
vi.mock('@/daemon/run', () => ({
  initialMachineMetadata: { host: 'host', platform: 'darwin', lynttyCliVersion: 'test', homeDir: '/tmp', lynttyHomeDir: '/tmp/.lyntty', lynttyLibDir: '/tmp/lyntty' },
}));
...
vi.mock('@/claude/utils/startLynttyServer', () => ({
  startLynttyServer: mocks.mockStartLynttyServer,
}));
...
describe('runAcp', () => {
  ...
  beforeEach(() => {
    ...
    mocks.mockStartLynttyServer.mockResolvedValue({
  ...
  it('updates session metadata with ACP config options (models and operating modes)', async () => {
    ...
    const baseMetadata = {
      ...
      lynttyLibDir: '/repo/.lyntty/lib',
      lynttyToolsDir: '/repo/.lyntty/tools',
    };
