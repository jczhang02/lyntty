...
import type { SessionMessage as WireSessionMessage } from 'lyntty-wire';
...
function handleApiError(err: unknown, context: string): never {
    if (err instanceof AxiosError) {
        ...
        if (status === 401) {
            throw new Error('Authentication expired. Run `lyntty remote auth login` to re-authenticate.');
        }
        if (status === 403) {
...
export async function postSessionUserMessage(
    config: Config,
    creds: Credentials,
    ...
    if (!localId || localId.length > 240) {
        throw new Error('localId must contain between 1 and 240 characters');
    }
    const content = {
        role: 'user',
        content: { type: 'text', text },
        localKey: localId,
        meta: { sentFrom: 'lyntty remote', ...meta },
    };
    const encrypted = encodeBase64(encrypt(session.encryption.key, session.encryption.variant, content));

    let data: { messages?: Array<{ id?: unknown; seq?: unknown; localId?: unknown; createdAt?: unknown; updatedAt?: unknown }> };
    try {
        const response = await axios.post(
            `${config.serverUrl}/v3/sessions/${encodeURIComponent(session.id)}/messages`,
            { messages: [{ content: encrypted, localId }] },
            { headers: authHeaders(creds) },
        );
        data = response.data as typeof data;
    } catch (error) {
        handleApiError(error, `persisting message for session ${session.id}`);
    }

    const persisted = data.messages?.find(message => message.localId === localId);
    if (
        !persisted
        ...
        throw new Error('Relay did not confirm the persisted message');
    }
    return {
        id: persisted.id,
        seq: persisted.seq,
