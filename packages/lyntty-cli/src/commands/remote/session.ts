...
export class SessionClient extends EventEmitter {
    ...
    /**
     * Wait for the Pi turn caused by one persisted user message.
     *
     * The Relay echo carrying `localId` is the acceptance barrier. Turn events
     * observed before that barrier belong to earlier work and must not satisfy
     * this wait. After the barrier, only a matching turn-start/turn-end pair is
     * accepted; legacy idle/ready fallbacks are deliberately excluded because
     * they cannot prove causality.
     */
    waitForTurnCompletionAfter(localId: string, timeoutMs = 300_000): Promise<void> {
        return new Promise<void>((resolve, reject) => {
            let accepted = false;
            let activeTurnId: string | null = null;
            ...
            const timeout = setTimeout(() => {
                finish(new Error(`Timeout waiting for agent turn completion after message ${localId}`));
            }, timeoutMs);

            const onMessage = (message: { content: unknown; localId?: string | null }) => {
                if (!accepted) {
                ...
                if (activeTurnId != null && turnEvent.turnId === activeTurnId) {
                    finish();
                }
            };
