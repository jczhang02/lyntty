...
export async function authLogin(config: Config): Promise<void> {
    ...
    try {
        await axios.post(`${config.serverUrl}/v1/auth/account/request`, {
            ...
            headers: { 'X-Lyntty-Client': 'cli-control-plane/0.1.0' },
        });
    } catch (err) {
    ...
    const qrData = `lyntty:///account?${encodeBase64Url(keypair.publicKey)}`;
    console.log('');
    qrcode.generate(qrData, { small: true }, (code: string) => {
        console.log(code);
    });
    console.log('## Authentication');
    console.log('- Action: Scan this QR code with the Lyntty app');
...
export async function authStatus(config: Config): Promise<void> {
    ...
    if (creds) {
        ...
        console.log('- Action: Run `lyntty remote auth login` to authenticate.');
    }
}
