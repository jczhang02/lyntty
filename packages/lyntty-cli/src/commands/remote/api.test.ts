...
function makeConfig(): Config {
    return {
        ...
        homeDir: '/tmp/lyntty-test',
        credentialPath: '/tmp/lyntty-test/agent.key',
    };
}
...
describe('api', () => {
    ...
    describe('listSessions', () => {
        ...
        it('throws on 401 with re-authenticate message', async () => {
            ...
            await expect(listSessions(config, creds)).rejects.toThrow(
                'Authentication expired. Run `lyntty remote auth login` to re-authenticate.',
            );
        });
    ...
    describe('postSessionUserMessage', () => {
        it('persists one encrypted idempotent v3 message and validates the acknowledgement', async () => {
            const { raw, sessionKey } = makeRawSessionWithDataKey(creds, { path: '/repo' });
            const session = {
                id: raw.id,
                encryption: resolveSessionEncryption(raw, creds),
            };
            mockedAxios.post.mockResolvedValueOnce({
                data: {
                    messages: [{
            ...
            const result = await postSessionUserMessage(
                config,
                creds,
                session,
                'hello',
            ...
            const [url, body, request] = mockedAxios.post.mock.calls[0];
            expect(url).toBe(`https://test-server.example.com/v3/sessions/${raw.id}/messages`);
            expect(request).toEqual({ headers: authHeader });
            expect(body.messages[0].localId).toBe('remote:1');
            ...
            expect(decrypted).toEqual({
                ...
                localKey: 'remote:1',
                meta: { sentFrom: 'lyntty remote', permissionMode: 'yolo' },
            });
        });

        it('fails closed when the Relay does not confirm the localId', async () => {
            const raw = makeRawSessionLegacy(creds, { path: '/repo' });
            mockedAxios.post.mockResolvedValueOnce({
                data: { messages: [{ id: 'message-1', seq: 1, localId: 'other', createdAt: 1, updatedAt: 1 }] },
            });

            await expect(postSessionUserMessage(
                config,
                creds,
                { id: raw.id, encryption: resolveSessionEncryption(raw, creds) },
                'hello',
                'remote:expected',
            )).rejects.toThrow('Relay did not confirm the persisted message');
        });
    });
