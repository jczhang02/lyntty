...
describe('SessionClient', () => {
    ...
    describe('waitForTurnCompletionAfter', () => {
        it('ignores an existing turn end until this localId is accepted and a new turn starts', async () => {
            const opts = makeOptions();
            const client = new SessionClient(opts);
            let resolved = false;
            const completion = client.waitForTurnCompletionAfter('remote:follow-up', 1000).then(() => {
                resolved = true;
            });
            ...
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                ...
                {
                    role: 'session',
                    content: { turn: 'old-turn', ev: { t: 'turn-end', status: 'completed' } },
                },
                opts.sessionId,
                { seq: 1 },
            ));
            ...
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                ...
                opts.sessionId,
                { seq: 2, localId: 'remote:follow-up' },
            ));

            // A trailing end from the old turn after persistence is still not causal.
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                opts.encryptionKey,
                opts.encryptionVariant,
                {
            ...
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                ...
                {
                    role: 'session',
                    content: { turn: 'follow-up-turn', ev: { t: 'turn-start' } },
                },
                opts.sessionId,
                { seq: 4 },
            ));
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                ...
                {
                    role: 'session',
                    content: { turn: 'follow-up-turn', ev: { t: 'turn-end', status: 'completed' } },
                },
                opts.sessionId,
                { seq: 5 },
            ));
        ...
        it('fails closed on disconnect after the localId barrier', async () => {
            const opts = makeOptions();
            const client = new SessionClient(opts);
            const completion = client.waitForTurnCompletionAfter('remote:disconnect', 1000);
            ...
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                ...
                { role: 'user', content: { type: 'text', text: 'follow up' } },
                opts.sessionId,
                { seq: 1, localId: 'remote:disconnect' },
            ));
            mockSocketInstance!.simulateServerEvent('disconnect', 'transport close');

            await expect(completion).rejects.toThrow('Socket disconnected while waiting for agent turn completion');
        });

        it('fails closed when no causally related turn completes before timeout', async () => {
            const opts = makeOptions();
            const client = new SessionClient(opts);
            const completion = client.waitForTurnCompletionAfter('remote:timeout', 20);
            ...
            mockSocketInstance!.simulateServerEvent('update', makeEncryptedUpdate(
                ...
                { role: 'user', content: { type: 'text', text: 'follow up' } },
                opts.sessionId,
                { seq: 1, localId: 'remote:timeout' },
            ));

            await expect(completion).rejects.toThrow('Timeout waiting for agent turn completion after message remote:timeout');
            client.close();
        });
    });
