 ...
 * - connect codex: Store OpenAI API key in Lyntty cloud
 * - connect claude: Store Anthropic API key in Lyntty cloud
 * - connect gemini: Store Gemini API key in Lyntty cloud
...
function showConnectHelp(): void {
    console.log(`
${chalk.bold('lyntty connect')} - Connect AI vendor API keys to Lyntty cloud

${chalk.bold('Usage:')}
  lyntty connect codex        Store your Codex API key in Lyntty cloud
  lyntty connect claude       Store your Anthropic API key in Lyntty cloud
  lyntty connect gemini       Store your Gemini API key in Lyntty cloud
  lyntty connect status       Show connection status for all vendors
  lyntty connect help         Show this help message

${chalk.bold('Description:')}
  The connect command allows you to securely store your AI vendor API keys
  in Lyntty cloud. This enables you to use these services through Lyntty
...
${chalk.bold('Notes:')}
  • You must be authenticated with Lyntty first (run 'lyntty auth login')
  • API keys are encrypted and stored securely in Lyntty cloud
  • You can manage your stored keys at app.lyntty.engineering
`);
}

async function handleConnectVendor(vendor: 'codex' | 'claude' | 'gemini', displayName: string): Promise<void> {
    console.log(chalk.bold(`\n🔌 Connecting ${displayName} to Lyntty cloud\n`));

    // Check if authenticated
    const credentials = await readCredentials();
    if (!credentials) {
        console.log(chalk.yellow('⚠️  Not authenticated with Lyntty'));
        console.log(chalk.gray('  Please run "lyntty auth login" first'));
        process.exit(1);
    }
...
async function handleConnectStatus(): Promise<void> {
    ...
    console.log('');
    console.log(chalk.gray('To connect a vendor, run: lyntty connect <vendor>'));
    console.log(chalk.gray('Example: lyntty connect gemini'));
    console.log('');
}

/**
 * Update local Gemini credentials file to keep in sync with Lyntty cloud
 * This ensures the Gemini SDK uses the same account as Lyntty
 */
