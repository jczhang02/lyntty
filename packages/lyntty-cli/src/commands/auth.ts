...
async function handleAuthLogin(args: string[]): Promise<void> {
  ...
  if (!forceAuth) {
    ...
    if (existingCreds && settings?.machineId) {
      ...
      console.log(chalk.gray(`  Use 'lyntty auth login --force' to re-authenticate`));
      return;
...
async function handleAuthLogout(): Promise<void> {
  ...
  console.log(chalk.yellow('⚠️  You will need to re-authenticate to use Lyntty again'));
  ...
  if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
    try {
      ...
      // Remove entire lyntty directory (as current logout does)
      if (existsSync(lynttyDir)) {
        rmSync(lynttyDir, { recursive: true, force: true });
      }
...
async function handleAuthStatus(): Promise<void> {
  ...
  if (settings?.machineId) {
    ...
    console.log(chalk.gray('  Run "lyntty auth login --force" to fix this'));
  }
