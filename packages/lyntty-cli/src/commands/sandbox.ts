...
export async function handleSandboxStatus(): Promise<void> {
    ...
    if (!config) {
        console.log('Sandbox is not configured. Run `lyntty sandbox configure`.');
        return;
    }
...
export function handleSandboxHelp(): void {
    console.log(`
${chalk.bold('lyntty sandbox')} - Sandbox management

${chalk.bold('Usage:')}
  lyntty sandbox configure      Configure sandbox settings interactively
  lyntty sandbox status         Show current sandbox configuration
  lyntty sandbox disable        Disable sandboxing
  lyntty sandbox help           Show this help
`);
}
