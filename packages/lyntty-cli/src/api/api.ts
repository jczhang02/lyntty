...
export class ApiClient {
  ...
  async getOrCreateSession(opts: {
    ...
    if (this.credential.encryption.type === 'dataKey') {
      ...
      // const contentDataKey = await deriveKey(this.secret, 'Lyntty EnCoder', ['content']);
    ...
    try {
      ...
      if (axios.isAxiosError(error) && error.response?.status) {
        ...
        if (status === 403 || status === 409) {
          ...
          console.log(chalk.yellow(
            `   → Run 'lyntty doctor clean' to reset local state and generate a new machine ID`
          ));
          console.log(chalk.yellow(
        ...
        if (status === 401) {
          throw new Error('Lyntty authentication expired or the relay data was reset. Run `lyntty auth login --force`, then `lyntty daemon start`.');
        }
