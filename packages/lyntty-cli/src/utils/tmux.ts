 ...
 * Ensures consistent tmux handling across lyntty-cli with proper session naming
 */
...
export class TmuxUtilities {
    ...
    public static readonly DEFAULT_SESSION_NAME = "lyntty";
    ...
    async spawnInTmux(
        ...
        try {
            ...
            // - undefined: Use first existing session or create "lyntty"
            // - empty string: Use first existing session or create "lyntty"
            ...
            if (!sessionName) {
                ...
                if (listResult && listResult.returncode === 0 && listResult.stdout.trim()) {
                    ...
                    // No sessions exist, create "lyntty"
                    sessionName = 'lyntty';
                    logger.debug(`[TMUX] No existing sessions, using default: ${sessionName}`);
                }
            }

            const windowName = options.windowName || `lyntty-${Date.now()}`;
