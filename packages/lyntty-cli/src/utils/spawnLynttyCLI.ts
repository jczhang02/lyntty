/**
 * Cross-platform Lyntty CLI spawning utility
 *
 * ## Background
 *
 ...
 * with no warning output when using Lyntty.
 *
 * ## The Wrapper Strategy
 *
 * We created a wrapper script `bin/lyntty.mjs` with a shebang `#!/usr/bin/env node`.
 * This allows direct execution on Unix systems and NPM automatically generates
 * Windows-specific wrapper scripts (`lyntty.cmd` and `lyntty.ps1`) when it sees
 ...
 *
 * **Unix/Linux/macOS:**
 * 1. User runs `lyntty` command
 * 2. Shell directly executes `bin/lyntty.mjs` (shebang: `#!/usr/bin/env node`)
 * 3. `bin/lyntty.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 *
 * **Windows:**
 * 1. User runs `lyntty` command
 * 2. NPM wrapper (`lyntty.cmd`) calls `node bin/lyntty.mjs`
 * 3. `bin/lyntty.mjs` either execs `node --no-warnings --no-deprecation dist/index.mjs` or imports `dist/index.mjs` directly
 *
 * ## The Spawning Problem
 *
 * When our code needs to spawn Lyntty cli as a subprocess (for daemon processes),
 * we were trying to execute `bin/lyntty.mjs` directly. This fails on Windows
...
/**
 * Spawn the Lyntty CLI with the given arguments in a cross-platform way.
 *
 * This function bypasses the wrapper script (bin/lyntty.mjs) and spawns the
 * actual CLI entrypoint (dist/index.mjs) directly with Node.js, ensuring
 * compatibility across all platforms including Windows.
 *
 * @param args - Arguments to pass to the Lyntty CLI
...
export function spawnLynttyCLI(args: string[], options: SpawnOptions = {}): ChildProcess {
  ...
  // bypassing the 'lyntty' wrapper that would normally be found in the shell's PATH.
  // However, we log it as 'lyntty' here because other engineers are typically looking
  // for when "lyntty" was started and don't care about the underlying node process
  // details and flags we use to achieve the same result.
  const fullCommand = `lyntty ${args.join(' ')}`;
  logger.debug(`[SPAWN LYNTTY CLI] Spawning: ${fullCommand} in ${directory}`);
  ...
  if (!existsSync(entrypoint)) {
    ...
    logger.debug(`[SPAWN LYNTTY CLI] ${errorMessage}`);
