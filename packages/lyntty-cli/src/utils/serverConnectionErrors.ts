...
export interface OfflineReconnectionConfig<TSession> {
    /** Server URL to health-check against (e.g., 'https://api.lyntty-relays.com') */
    serverUrl: string;
    ...
    const attemptReconnect = async () => {
        ...
        try {
            ...
            if (axios.isAxiosError(e) && e.response?.status === 401) {
                ...
                config.onNotify('❌ Authentication failed. Please re-authenticate with `lyntty auth`.');
...
export const ERROR_DESCRIPTIONS: Record<string, string> = {
    ...
    // HTTP errors
    '401': 'authentication failed - run `lyntty auth`',
...
class OfflineState {
    ...
    private print(): void {
        const summary = [...this.failures.values()]
            ...
            .join('; ');
        console.log(`⚠️  Lyntty relay unreachable, offline mode with auto-reconnect enabled - error details: ${summary}`);
