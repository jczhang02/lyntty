...
import { resolveLynttySession, type ResumableLynttySession } from './resolveLynttySession';
...
export function parseResumeCommandArgs(args: string[]): { showHelp: boolean; sessionId: string } {
    ...
    if (args.length > 1) {
        throw new Error(`Unexpected arguments for lyntty resume: ${args.slice(1).join(' ')}`);
    }
...
export function buildResumeLaunch(session: ResumableLynttySession, options: ResumeLaunchOptions = {}): ResumeLaunch {
    const { metadata } = session;
    const flavor = resolveFlavor(metadata);

    if (flavor === 'codex') {
        if (!metadata.codexThreadId) {
            throw new Error(`Lyntty session ${session.id} is missing its Codex thread ID.`);
        }
    ...
    if (flavor === 'claude') {
        if (!metadata.claudeSessionId) {
            throw new Error(`Lyntty session ${session.id} is missing its Claude session ID.`);
        }
        const args = ['claude'];
        if (options.claudeStartingMode) {
            args.push('--lyntty-starting-mode', options.claudeStartingMode);
        }
        if (options.startedBy) {
            args.push('--started-by', options.startedBy);
        }
    ...
    throw new Error(`Lyntty session ${session.id} uses unsupported flavor "${metadata.flavor ?? 'unknown'}".`);
}

export function formatResumeHelp(): string {
    return [
        'lyntty resume - Resume a previous Lyntty session',
        '',
        'Usage:',
        '  lyntty resume <lyntty-session-id>',
        '',
        'Examples:',
        '  lyntty resume cmmij8olq00dp5jcxr3wtbpau',
        '  lyntty resume cmmij8',
        '',
...
function spawnResumeChild(launch: ResumeLaunch): Promise<number | null> {
    return new Promise((resolve, reject) => {
        const child = spawnLynttyCLI(launch.args, {
            cwd: launch.cwd,
            env: process.env,
            stdio: 'inherit',
        });
...
export async function handleResumeCommand(args: string[]): Promise<void> {
    ...
    const session = await resolveLynttySession(parsed.sessionId);
