 ...
 * for lyntty CLI including configuration, daemon status, logs, and links
 */
...
export function getEnvironmentInfo(): Record<string, any> {
    return {
        ...
        LYNTTY_PROJECT_ROOT: process.env.LYNTTY_PROJECT_ROOT,
        DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING: process.env.DANGEROUSLY_LOG_TO_SERVER_FOR_AI_AUTO_DEBUGGING,
        NODE_ENV: process.env.NODE_ENV,
        DEBUG: process.env.DEBUG,
        workingDirectory: process.cwd(),
        processArgv: process.argv,
        lynttyDir: configuration?.lynttyHomeDir,
...
/**
 * Slim daemon status output for `lyntty daemon status`
 */
export async function runDoctorDaemon(): Promise<void> {
    ...
    console.log(chalk.gray('\nRun `lyntty doctor` for full diagnostics.\n'));
}
...
export async function runDoctorCommand(): Promise<void> {
    ...
    // All Lyntty processes
    try {
        const allProcesses = await findAllLynttyProcesses();
        if (allProcesses.length > 0) {
            console.log(chalk.bold('🔍 All Lyntty CLI Processes'));
            ...
            if (allProcesses.length > 1) {
                console.log(chalk.bold('\n💡 Process Management'));
                console.log(chalk.gray('To clean up runaway processes: lyntty doctor clean'));
            }
        } else {
            console.log(chalk.red('❌ No lyntty processes found'));
        }
    } catch (error) {
    ...
    console.log(`LYNTTY_HOME_DIR: ${env.LYNTTY_HOME_DIR ? chalk.green(env.LYNTTY_HOME_DIR) : chalk.gray('not set')}`);
    console.log(`LYNTTY_SERVER_URL: ${env.LYNTTY_SERVER_URL ? chalk.green(env.LYNTTY_SERVER_URL) : chalk.gray('not set')}`);
    ...
    // Configuration
    console.log(chalk.bold('\n⚙️  Configuration'));
    console.log(`Lyntty Home: ${chalk.blue(configuration.lynttyHomeDir)}`);
