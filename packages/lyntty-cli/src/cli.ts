...
import { handleSelfCommand, handleUpdateCommand } from './commands/update';
...
import { createDaemonServiceManager } from './daemon/service';
...
${chalk.bold('Usage:')}
  ...
  lyntty update            Inspect, apply, or roll back a verified release
...
async function handleDaemonCommand(args: string[]): Promise<void> {
  ...
  if (daemonSubcommand === 'start') {
    ...
    if (runtimeLayout().compiled) {
      ...
      if (state === 'not-installed') {
        throw new Error('lynttyd user service is not installed. Run `lyntty daemon install` first.');
      }
  ...
  if (daemonSubcommand === 'stop') {
    if (runtimeLayout().compiled && process.env.LYNTTY_INSTALL_ROOT) {
      const manager = createDaemonServiceManager();
      if (await manager.status() !== 'not-installed') {
        await manager.stop();
        return;
      }
    }
  ...
  if (daemonSubcommand === 'install') {
    if (args.length > 1) throw new Error('Usage: lyntty daemon install');
    await install();
    return;
  }
  if (daemonSubcommand === 'uninstall') {
    if (args.length > 1) throw new Error('Usage: lyntty daemon uninstall');
    await uninstall();
    return;
  }
...
${chalk.bold('Usage:')}
  ...
  lyntty daemon install            Repair the managed lynttyd user service
...
async function run(): Promise<void> {
  ...
  switch (subcommand) {
    ...
    case 'update':
      ...
      return;
    case 'self':
      await handleSelfCommand(args.slice(1));
      return;
    case 'bye':
      console.log('Bye!');
      return;
