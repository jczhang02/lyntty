...
const binPath = resolve(packageDir, 'bin', 'lyntty-agent.mjs');
const keepIntegrationEnv = ['1', 'true', 'yes'].includes((process.env.LYNTTY_AGENT_KEEP_ENV ?? '').toLowerCase());
...
function createGitProject(envDir: string): { projectDir: string; worktreeDir: string } {
    const projectDir = join(envDir, 'lyntty-agent-test-project');
    const worktreeDir = join(projectDir, '.dev', 'worktree', 'feature-branch');

    mkdirSync(projectDir, { recursive: true });
    runCommand('git', ['init', '--initial-branch=main'], projectDir);
    runCommand('git', ['config', 'user.name', 'Lyntty Agent Test'], projectDir);
    runCommand('git', ['config', 'user.email', 'lyntty-agent-tests@example.com'], projectDir);

    writeFile(join(projectDir, 'README.md'), '# Lyntty Agent Test Project\n');
...
async function waitForSessionInList(sessionId: string, env: NodeJS.ProcessEnv): Promise<void> {
    ...
    }, 20_000, `session ${sessionId} to appear in lyntty-agent list`);
}
...
async function approveAgentLogin(
    ...
    const response = await fetch(`${serverUrl}/v1/auth/account/response`, {
        ...
        headers: {
            ...
            'X-Lyntty-Client': 'cli-control-plane/0.1.0',
        },
        body: JSON.stringify({
...
async function runAgentAuthLogin(env: NodeJS.ProcessEnv, approval: { serverUrl: string; token: string; secret: Uint8Array }): Promise<string> {
    return await new Promise<string>((resolvePromise, rejectPromise) => {
        ...
        const timeout = setTimeout(() => {
            ...
            settled = true;
            child.kill('SIGKILL');
            rejectPromise(new Error(`Timed out waiting for lyntty-agent auth login.\n${stdout}\n${stderr}`));
        }, 60_000);
        ...
        child.on('close', code => {
            if (code !== 0) {
                finish(new Error(`lyntty-agent auth login exited with code ${code}\n${stdout}\n${stderr}`));
                return;
            }
...
async function listDaemonSessions(httpPort: number): Promise<Array<{ lynttySessionId: string; pid: number; startedBy: string }>> {
    ...
    const parsed = await response.json() as { children: Array<{ lynttySessionId: string; pid: number; startedBy: string }> };
...
describe('lyntty-agent integration', { timeout: 180_000 }, () => {
    beforeAll(async () => {
        ...
        if (keepIntegrationEnv) {
            console.log(`[lyntty-agent integration] keeping environment: ${integrationEnvName}`);
            console.log(`[lyntty-agent integration] environment dir: ${integrationEnvDir}`);
        }
    });
    ...
    it('authenticates, lists machines, and spawns a session through the real daemon RPC path', async () => {
        ...
        await waitFor(async () => {
            ...
            return refreshedMachine?.metadata?.resumeSupport?.rpcAvailable === true
                && refreshedMachine.metadata.resumeSupport.lynttyAgentAuthenticated === true;
    ...
    it('spawns in the test project root and sends a message through lyntty-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testProjectDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'lyntty-agent root message';
    ...
    it('spawns in a git worktree and sends a message through lyntty-agent CLI', async () => {
        if (!activeMachineId || !integrationConfig || !agentHomeDir || !testWorktreeDir) {
            throw new Error('Integration environment not initialized');
        }

        const agentEnv = agentEnvVars(integrationConfig.serverPort, agentHomeDir);
        const prompt = 'lyntty-agent worktree message';
        const spawnResult = parseJson<{
            type: 'success' | 'requestToApproveDirectoryCreation' | 'error';
            sessionId?: string;
            machineId?: string;
