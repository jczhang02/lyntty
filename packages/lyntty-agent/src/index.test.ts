...
function runCli(...args: string[]): { stdout: string; stderr: string; exitCode: number } {
    try {
        const stdout = execFileSync(process.execPath, [
            ...
            binPath,
            ...args,
        ], { encoding: 'utf-8', env: { ...process.env, LYNTTY_HOME_DIR: '/tmp/nonexistent-lyntty-test' } });
...
describe('lyntty-agent CLI', () => {
    it('should display help output', () => {
        const { stdout } = runCli('--help');
        expect(stdout).toContain('lyntty-agent');
        expect(stdout).toContain('CLI client for controlling Lyntty Coder agents remotely');
    });
