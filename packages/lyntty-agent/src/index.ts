...
const SUPPORTED_AGENTS: SupportedAgent[] = ['pi', 'claude', 'codex', 'gemini', 'openclaw'];
...
function ensureMachineCanResume(machine: DecryptedMachine): void {
    const metadata = (machine.metadata ?? {}) as {
        resumeSupport?: {
            ...
            lynttyAgentAuthenticated?: unknown;
        };
    };
    ...
    if (metadata.resumeSupport?.lynttyAgentAuthenticated === false) {
        throw new Error('Resume is unavailable on this machine. Run `lyntty-agent auth login` in that machine environment first.');
    }
...
program
    .name('lyntty-agent')
    .description('CLI client for controlling Lyntty Coder agents remotely')
    .version('0.1.0');
